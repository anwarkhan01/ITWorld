...
const importProducts = asyncHandler(async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(500, "Error occurred while importing products", err.message)
    );
  }
});
...
const getRandomProducts = asyncHandler(async (req, res, next) => {
  ...
  res.json(
    new ApiResponse(200, "Random products fetched successfully", {
      products: products,
...
const getFilterProducts = asyncHandler(async (req, res, next) => {
  ...
  const [products, total] = await Promise.all([
    Product.find(filters).sort(sortObj).skip(skip).limit(limitNum).lean(),
    Product.countDocuments(filters),
  ]);

  res.json(
    new ApiResponse(200, "Filtered Product Fetched Successfully", {
      count: products.length,
      products,
      pagination: {
        page: pageNum,
...
const getCategorizedProducts = asyncHandler(async (req, res, next) => {
  ...
  res.json(
    new ApiResponse(200, "Categorized Products Fetched Successfully", {
      category,
      products,
