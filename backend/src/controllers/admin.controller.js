...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  //   update["meta.processingAt"] = now;
  // }

  if (finalStatus === "ready") {
    update["pickup.readyAt"] = now;
    update["cancelled.isCancelled"] = false;
    update["pickup.isAvailableAtStore"] = true;
  }
  if (finalStatus === "pickedup") {
    update["pickup.pickedUpAt"] = now;
    update["cancelled.isCancelled"] = false;
    update["pickup.isAvailableAtStore"] = false;
  }

  if (finalStatus === "shipped") {
    update["cancelled.isCancelled"] = false;
    update["meta.shippedAt"] = now;
  }

  if (finalStatus === "delivered") {
    update.deliveredOn = now;
    update["cancelled.isCancelled"] = false;
    update["pickup.pickedUpAt"] = now;
  }

  if (finalStatus === "cancelled") {
    update["cancelled.isCancelled"] = true;
    update["cancelled.cancelledAt"] = now;
    update["pickup.isAvailableAtStore"] = false;
    update["pickup.isAvailableAtStore"] = false;
  }

  if (finalStatus === "refunded") {
    update["cancelled.refunded"] = true;
    update["cancelled.refundDate"] = now;
    update["cancelled.isCancelled"] = true;
    update["pickup.isAvailableAtStore"] = false;
  }
