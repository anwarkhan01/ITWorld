...
const createOrder = asyncHandler(async (req, res, next) => {
  ...
  readyAt.setUTCDate(readyAt.getUTCDate() + 5);
...
const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  if (!orderId) {
    return next(
      new ApiError(400, "OrderId is required for cancellation of the order")
    );
  }
  ...
  if (order.status === "cancelled") {
    return next(new ApiError(400, "Order already cancelled"));
  }

  const cancelledAt = new Date();
  const refundDate = new Date(cancelledAt);
  refundDate.setDate(refundDate.getDate() + 7);

  const refundId = `RFND_${Date.now()}`; // placeholder; replace with gateway ID later
